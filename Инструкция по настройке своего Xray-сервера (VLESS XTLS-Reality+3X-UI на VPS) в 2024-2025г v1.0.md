...
Большую часть информации я брал из замечательных статей Deleted-user (он же MiraclePtr) с habr.com. Если у вас нет возможности прочитать их в браузере (в РФ они заблокированы), можете скачать их с [моей страницы](https://github.com/EmptyLibra/Configure-Xray-with-VLESS-Reality-on-VPS-server) на github в виде html страниц.

- [i] [Статьи](https://habr.com/ru/articles/727868/) пользователя Deleted-user (раньше MiraclePtr, видимо, его банят периодически) на habr.com от 2023г о различных, устойчивых к блокировке протоколов и их настройке на собственном сервере (тут много теории и технических аспектов, крайне рекомендую, если интересует, как всё устроено изнутри, увы, статьи заблокированы в РФ). Там цикл из 12 статей, вот ссылки на все:
	- [Раз](https://habr.com/ru/articles/710980/) - краткая эволюция протоколов, устойчивых к блокировкам и способов их выявления
	- [Два](https://habr.com/ru/articles/727868/) - современные способы обхода блокировок и их происхождение (V2Ray, XRay, XTLS, Hysteria, Cloak...)
	- [Три](https://habr.com/ru/articles/728696/) - обзор программ-клиентов для связи с сервером по защищённым протоколам
	- [Четыре](https://habr.com/ru/articles/728836/) - настройка сервера XRay для Shadowsocks-2022 и VLESS-XTLS-Vision
	- [Пять](https://habr.com/ru/articles/731608/) - настройка сервера и клиента Xray с VLESS-XTLS-Reality
	- [Шесть](https://habr.com/ru/articles/735536/) - настройка сервера через графическую панель 3X-UI
	- [Семь](https://habr.com/ru/articles/761798/) - настройка подключения к серверу через CDN (Websocket, grpc)
	- [Восемь](https://habr.com/ru/articles/770400/) - частые вопросы/ответы по прошлым статьям и комментариям
	- [Девять](https://habr.com/ru/articles/774838/) - получение доступа к корпоративной сети с помощью Xray (проброс портов, реверс-прокси)
	- [Десять](https://habr.com/ru/articles/776256/) - недектируемый VPN (именно VPN, а не прокси) OpenConnect и его настройка
	- [Одиннадцать](https://habr.com/ru/articles/777656/) - GOST: швейцарский нож для туннелирования и обхода блокировок
	- [Двенадцать](https://habr.com/ru/articles/778134/) - Domain fronting для чайников, и как его использовать для обхода блокировок
	- [Тринадцать](https://habr.com/ru/articles/799751/) - подробная статья с полной настройкой VPS сервера под неблокируемые протоколы 2024 
	- [Четырнадцать](https://habr.com/ru/articles/834290/) - Малоизвестные фичи XRay, о которых невозможно молчать
	- [Пятнадцать](https://habr.com/ru/articles/837904/) - Matrix: децентрализованные открытые мессенджеры с E2E-шифрованием
...
- [i] [Крайне грамотная статья](https://habr.com/ru/articles/799751/) от пользователя Deleted-user с habr.com по настройке VLESS-XTLS-Reality/ShadowSocks и многое другое в 2024г.
- [i] [Статья](https://habr.com/ru/articles/785186/) пользователя quakin на habr.com с инструкцией по настройке VLESS-XTLS-Reality через панель 3X-UI, CDN и прочее.
- [i] [Инструкция](https://github.com/XTLS/Xray-core/discussions/3518) из обсуждения Xray на Github по настройке VLESS XTLS-Reality с подробным разбором механизмов его работы.
...
- [i] Простенькая [инструкция](https://pikabu.ru/story/nastroyka_luchshikh_vpn_protokolov_vless_s_xtlsreality_na_servere_xray_s_ustanovkoy_cherez_3xui_panel_10782662) с Пикабу о настройке Vless-xtls-Reality через 3X-UI (есть проблемы с безопасностью).
...
- **Запись** - это неупорядоченное множество пар **ключ**:**значение**, заключённое в фигурные скобки _«_{ }_»_. Ключ описывается **строкой**, между ним и значением стоит символ _«_:_»_. Пары _ключ-значение_ отделяются друг от друга запятыми.
  Например: `"outbound": "proxy"`.
- **Массив** (одномерный) — это упорядоченное множество **значений**. Массив заключается в квадратные скобки _«_[ ]_»_. Значения разделяются запятыми. Массив может быть пустым, то есть не содержать ни одного значения. Значения в пределах одного массива могут иметь разный тип. 
  Например: `"rules": [ ... ]`.
